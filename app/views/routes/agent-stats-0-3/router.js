...
router.post(`/timeselection-preset-router`, (req, res) => {
  const timeselectionPreset = req.session.data['timeselection-preset']

  if (timeselectionPreset == 'custom') {
    res.redirect(`timeselection`)
  }
   else {
    res.redirect(`service-stats`)
  }
})
